            ...
            {mockCertificates.map((cert, index) => (
              ...
              >
                ...
                />
                  ...
                  <h3 className="text-white font-bold text-base mb-2 line-clamp-2 pr-16">{cert.title}</h3>
                  <p className="text-white/90 text-sm mb-1 pr-16">{cert.issuer}</p>
                  <p className="text-white/70 text-xs mb-3 pr-16">{cert.date}</p>
                  
                  {/* View Credentials Button - Fixed Position */}
                  <Button
                    size="sm"
                    variant="outline"
                    className="absolute bottom-3 right-3 w-16 h-6 text-xs bg-green-500/20 border-green-400/30 text-green-300 hover:bg-green-500/30 hover:border-green-400/50 hover:text-green-200 transition-all duration-200 z-10"
                    onClick={(e) => {
                      e.stopPropagation();
